...
func (p *StreamPacket) Clone() *StreamPacket {
	...
	packet.Type = p.Type
	packet.VideoFrameType = p.VideoFrameType
	packet.VideoCodecID = p.VideoCodecID
	packet.AudioFormat = p.AudioFormat
	packet.SamplingRate = p.SamplingRate
	packet.SampleLength = p.SampleLength
	packet.AudioType = p.AudioType
	packet.Payload = p.Payload
	return packet
}

func (p *StreamPacket) String() {
/*
